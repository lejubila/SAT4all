...
return [
    ...
    'mac_lookup' => [
        ...
        // ── Virtual context ───────────────────────────────────────────────────
        'virtual_section'        => 'Virtual Context',
        'virtual_badge_certain'  => 'Virtual',
        'virtual_badge_probable' => 'Probably virtual',
        'virtual_platform_label' => 'Platform',
        'virtual_uses_label'     => 'Typical use',

        'virtual_desc_vmware'          => 'This OUI is assigned by IEEE to VMware. It almost certainly identifies a network interface belonging to a VMware virtual machine.',
        'virtual_desc_vmware_esxi'     => 'The 00:50:56 prefix with fourth octet ≥ 40 is auto-generated by VMware ESXi / vSphere for VMs hosted on the cluster.',
        'virtual_desc_virtualbox'      => 'The OUI 08:00:27 is assigned by IEEE to Oracle for VirtualBox. It identifies a VM running in VirtualBox on Windows, Linux or macOS.',
        'virtual_desc_hyper_v'         => 'OUI reserved by Microsoft for Hyper-V. Used by Hyper-V VMs on Windows Server and desktop, and by WSL 2 virtual network adapters.',
        'virtual_desc_azure'           => 'OUI used by Microsoft Azure for cloud VM network interfaces. Indicates a virtual NIC attached to a VM hosted on Azure.',
        'virtual_desc_xen'             => 'OUI reserved for Xen Hypervisor and Citrix XenServer. Also historically used by AWS EC2 on Xen-based instances (pre-Nitro era).',
        'virtual_desc_parallels'       => 'OUI assigned to Parallels for VMs on macOS (Parallels Desktop). Indicates a Windows, Linux or other guest VM hosted on a Mac.',
        'virtual_desc_qemu_kvm'        => 'The 52:54:00 prefix is the de-facto standard for QEMU/KVM. Used by libvirt, Proxmox VE, oVirt, and any KVM-based stack. The L bit is set (locally administered) by convention.',
        'virtual_desc_docker'          => 'The 02:42 prefix is assigned by Docker to container interfaces. The docker0 bridge uses this range to distinguish container traffic from physical traffic.',
        'virtual_desc_la_generic'      => 'The "Locally Administered" (L) bit is set in the first octet: this address was not assigned by IEEE but generated or configured locally. Several scenarios can produce it.',

        'virtual_uses_vmware'          => 'VMware Workstation or Fusion VM|VM on VMware ESXi / vSphere|VM templates and clones',
        'virtual_uses_vmware_esxi'     => 'VM managed by VMware ESXi or vCenter|Snapshots and clones on a vSphere cluster|VMs created via vSphere Web Client or API',
        'virtual_uses_virtualbox'      => 'VirtualBox VM on Windows, Linux or macOS host|Isolated development and test environments|Virtual labs and snapshots',
        'virtual_uses_hyper_v'         => 'Hyper-V VM on Windows Server or Windows 10/11|Hyper-V internal or external virtual switch|WSL 2 network adapter (Windows Subsystem for Linux)',
        'virtual_uses_azure'           => 'Azure VM virtual NIC (IaaS)|Azure Virtual Network interface|Azure VM with accelerated networking (SR-IOV)',
        'virtual_uses_xen'             => 'VM on Xen Hypervisor or Citrix XenServer|AWS EC2 instance on Xen infrastructure (pre-Nitro)|OpenStack compute node using Xen',
        'virtual_uses_parallels'       => 'Windows or Linux VM on Parallels Desktop for Mac|Cross-platform development environment on macOS|VM with access to shared macOS hardware devices',
        'virtual_uses_qemu_kvm'        => 'KVM VM managed by libvirt (virt-manager, virsh)|Node in a Proxmox VE cluster|VM on oVirt / Red Hat Virtualization|QEMU emulation for non-x86 architectures',
        'virtual_uses_docker'          => 'Docker container attached to the default bridge (docker0)|veth interface of a container in a custom bridge network|Docker Compose container with an internal network',
        'virtual_uses_la_generic'      => 'Wi-Fi MAC randomisation (iOS 14+, Android 10+, Windows 10/11)|VPN or tunnel interface (OpenVPN, WireGuard, GRE)|SDN virtual switch (Open vSwitch, Linux bridge)|Linux bonding/teaming with a locally-generated MAC|MAC address manually configured by an administrator',
    ],
