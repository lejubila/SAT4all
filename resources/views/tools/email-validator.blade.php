...
@section('title', __('tools.email_validator.title'))

@section('content')
    <section class="mb-6">
        <h1 class="text-2xl font-bold tracking-tight text-slate-900">
            {{ __('tools.email_validator.title') }}
        </h1>
        <p class="mt-2 max-w-2xl text-slate-600">
            {{ __('tools.email_validator.description') }}
        </p>
    </section>

    <form hx-post="{{ route('tools.email-validator.check') }}"
          hx-target="#result"
          hx-swap="innerHTML"
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end">
            <div class="flex-1">
                <label class="mb-1 block text-sm font-medium text-slate-700">
                    {{ __('tools.email_validator.label_email') }}
                </label>
                <input type="text"
                       name="email"
                       placeholder="{{ __('tools.email_validator.placeholder_email') }}"
                       autocomplete="off"
                       spellcheck="false"
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end">
            <div>
                <p class="mb-2 text-xs font-medium uppercase tracking-wide text-slate-500">
                    {{ __('tools.email_validator.label_captcha') }}
                </p>
            ...
            <div class="flex-1 sm:max-w-xs">
                ...
                <input type="text"
                       ...
                       spellcheck="false"
                       maxlength="6"
                       placeholder="{{ __('tools.email_validator.placeholder_captcha') }}"
            ...
            <button type="submit"
                ...
                {{ __('tools.email_validator.btn_check') }}
            </button>
        </div>

        <p class="text-xs text-slate-400">{{ __('tools.email_validator.hint') }}</p>
    </form>

    <div id="result">
        <p class="text-sm text-slate-400">{{ __('tools.email_validator.empty') }}</p>
    </div>
@endsection
